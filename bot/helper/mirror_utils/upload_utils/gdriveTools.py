...
class GoogleDriveHelper:
    ...
    def __getIdFromUrl(link: str):
        if "folders" in link or "file" in link:
            ...
            if res is None:
                raise IndexError("G-Drive Id Bulunamadı.")
            return res.group(5)
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        msg = f"<b> {contents_count} Sonuc <i>{fileName}</i></b>"
        buttons = ButtonMaker()
        buttons.buildbutton("🔎 Göster", f"https://telegra.ph/{self.path[0]}")
