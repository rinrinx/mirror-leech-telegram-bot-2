...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>♻️ CL:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>⏫ UL:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                else:
                ...
                msg += f"\n<b>⚡️ Speed: </b>{get_readable_file_size(download.torrent_info().upspeed)}/s"
                msg += f" | <b>⏫ UL: </b>{get_readable_file_size(download.torrent_info().uploaded)}"
                msg += f"\n<b>♻️ Ratio: </b>{round(download.torrent_info().ratio, 3)}"
                msg += f" | <b>⏰ Time: </b>{get_readable_time(download.torrent_info().seeding_time)}"
                msg += f"\n\n\n❌ To Cancel<code>/{BotCommands.CancelMirror} {download.gid()}</code>"
            else:
                msg += f"\n<b>📝 Size: </b>{download.size()}"
            msg += "\n\n"
        ...
        bmsg = f"<b>C:</b> {cpu_percent()}% | <b>F:</b> {free}"
        ...
        if STATUS_LIMIT is not None and tasks > STATUS_LIMIT:
            ...
            buttons = ButtonMaker()
            buttons.sbutton("⏮️ Prev", "status pre")
